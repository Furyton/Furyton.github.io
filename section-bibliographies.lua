...
local function create_section_bibliography (div)
  ...
  local header = section_header(div)
  local refs_block = div.content:find_if(function (b)
      return b.attr and b.identifier == 'refs'
  end)
  ...
  local subsections
  local blocks
  if not header or section_refs_level > header.level or not refs_block then
    -- Don't do anything for lower level sections or sections without refs block.
    return nil
